...
export class ForceLengthxPipe implements PipeTransform {
  transform(value: string | null = null, x: number) {
    if (value && value.length > x) {
      return value.substr(0, x);
    } else if (value && value.length < x) {
...
export class TimerComponent implements OnChanges {
  ...
  @Input('reset')
  public reset$: Observable<boolean> = new Subject<boolean>(); // jz for utility when questionID has changed and inform timeSrc

  public startAgain$: Subject<boolean> = new Subject<boolean>();

  // anyway -> should be countdown
  public timeSource$ = timer(0, this.INTERVAL)
    .pipe(
      map((time: number) => {
        const date = time === 0 ? new Date(this.INTERVAL) : new Date(0); // first one needs to be offset
        if (this.timeToRun > (time + 1) * this.INTERVAL) { // wont exceed the timetoRun, time + 1 so one stpe in afacance to close properly at 0.00,
  ...
  public timer$ = defer(() => timer(0, this.timeToRun).pipe(
    tap((time) => {
      if (time === 0) {
        this.startTime = Date.now();
      }
    }),
    // because swithcmap runs it twice, for each emission, runnin 2 concurrent timers
    switchMap((time) => {
      if (time === 0) {
  ...
  public ngOnChanges(changes: SimpleChanges): void {
    if (changes.reset && this.reset$) {
      this.reset$.subscribe(() => this.startAgain$.next(true));
    }
  }
