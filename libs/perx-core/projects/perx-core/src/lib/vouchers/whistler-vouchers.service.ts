...
export class WhistlerVouchersService implements IVoucherService {
  ...
  public getAll(voucherParams?: IGetVoucherParams): Observable<IVoucher[]> {
    return this.http.get<IJsonApiListPayload<IWAssignedAttributes>>(this.vouchersUrl)
      .pipe(
        map((res) => res.data),
        mergeMap((vouchers: IJsonApiItem<IWAssignedAttributes>[]) => combineLatest(...vouchers.map(v => this.getFullVoucher(v)))),
