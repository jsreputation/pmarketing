...
export class WhistlerSurveyService {
  ...
  public static WSurveyToSurvey(survey: IJsonApiItemPayload<Partial<IWSurveyEngagementAttributes>>): any {
    ...
    if (dp) {
      const questions: IQuestion[] = dp.questions.map(q => {
        // @ts-ignore
        const payload: IPayload = { ...q.payload, type: WhistlerSurveyService.WQTypeToQType(q.payload.type) };
        return { ...q, payload };
      });
      return {
        id: survey.data.id,
    ...
    return this.campaignService.getCampaign(id)
      .pipe(
        ...
        map((res: IJsonApiItemPayload<IWSurveyEngagementAttributes>) => {
          const surveyData = WhistlerSurveyService.WSurveyToSurvey(res);
          const results: { [key: string]: ISurveyOutcome } = {};
          if (disProp && disProp.successPopUp) {
            results.outcome = WhistlerSurveyService.outcomeToGameOutcome(disProp.successPopUp);
          }
          if (disProp && disProp.noRewardsPopUp) {
            results.noOutcome = WhistlerSurveyService.outcomeToGameOutcome(disProp.noRewardsPopUp);
          }
          return {
            ...surveyData,
            results,
