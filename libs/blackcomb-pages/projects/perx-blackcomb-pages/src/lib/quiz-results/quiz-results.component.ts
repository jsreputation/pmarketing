...
export class QuizResultsComponent implements OnInit {
  ...
  public ngOnInit(): void {
    ...
    this.translate
      .get('QUIZ_TEMPLATE.QUESTION_TIME_TAKEN')
      .subscribe((text) => {
  ...
  public fetchTitle(): void {
    this.translate.get('QUIZ_TEMPLATE.SUMMARY_CORRECT').subscribe((text) => {
      this.title = text
        .replace('{{total_length}}', this.results.length)
        .replace('{{total_correct}}', this.correctAnswers);
    });
  }

  public fetchSubTitle(): void {
    const total = this.results.reduce(
      (sum, q) => sum + Math.floor(oc(q).time(0)),
      0
    );
    if (total === 0) {
      this.subTitle = '';
  ...
  public next(): void {
    ...
    if (!this.rewardsAcquired) {
      ...
      this.translate
        ...
        .subscribe((res: any) => {
          const noOutcomeTitle = res['QUIZ_TEMPLATE.NO_OUTCOME_SCORE'].replace(
            '{{points}}',
            points
          );
          this.popup = {
            ...
            text: oc(noOutcome).subTitle[this.currentLang].text(
              res['QUIZ_TEMPLATE.NO_OUTCOME_TXT']
            ),
            imageUrl: oc(noOutcome).image(),
            buttonTxt: oc(noOutcome).button[this.currentLang].text(
              res['QUIZ_TEMPLATE.NO_OUTCOME_CTA']
            ),
            /* eslint-enable */
          };
      ...
      this.translate
        ...
        .subscribe((res: any) => {
          const outcomeTitle = res[
            'QUIZ_TEMPLATE.POSITIVE_OUTCOME_TXT'
          ].replace('{{points}}', points);
          this.popup = {
            title: oc(outcome).title[this.currentLang].text(outcomeTitle),
            text: oc(outcome).subTitle[this.currentLang].text(
              res['QUIZ_TEMPLATE.POSITIVE_OUTCOME_REWARD']
            ),
            buttonTxt: oc(outcome).button[this.currentLang].text(
              res['QUIZ_TEMPLATE.POSITIVE_OUTCOME_CTA']
            ),
          ...
          if (
            ...
            this.prizeSetOutcomes &&
            this.prizeSetOutcomes.length > 0
          ) {
