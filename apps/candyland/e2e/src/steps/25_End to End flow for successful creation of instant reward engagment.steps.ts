...
import { ElementApp, CreateInstantRewardAppPage } from '../pages/candylandApp.po';
...
Given(/^8_I enter a test string in the headline message and sub-headline message for instant reward.$/, async () => {
  ...
  await Element.inputTextArray().get(1).sendKeys('E2Eteststring001');
  // Inputing test string in sub-headline
  await Element.inputTextArray().get(2).sendKeys('E2Eteststring002');
});
