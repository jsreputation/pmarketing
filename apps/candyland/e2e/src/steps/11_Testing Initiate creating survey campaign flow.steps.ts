...
Then(/^1_The campaign tab should be present.$/, async () => {
  ...
  await browser.wait(ec.elementToBeClickable(ElementApp.h3Array().get(3)), 5000);
  expect(await ElementApp.h3Array().get(3).getText()).to.be.equal('Campaigns');
});
...
When(/^2_I click on the campaign tab$/, async () => {
  ...
  await ElementApp.h3Array().get(3).click();
...
Given(/^3_I am on the campaign page$/, async () => {
  await CampaignAppPage.navigateToCampaign();
...
Then(/^3_I should see the relevant elements.$/, async () => {
  ...
  await browser.wait(ec.presenceOf(ElementApp.inputArray().get(0)), 6000);
  // waiting for start date filter to load
  await browser.wait(ec.presenceOf(ElementApp.inputArray().get(1)), 6000);
  // waiting for end date filter to load
  await browser.wait(ec.presenceOf(ElementApp.inputArray().get(2)), 6000);
  // doing the assertions for the elements present
  expect(await ElementApp.inputArray().get(0).isPresent()).to.equal(true);
  expect(await ElementApp.inputArray().get(1).isPresent()).to.equal(true);
  expect(await ElementApp.inputArray().get(2).isPresent()).to.equal(true);
});
