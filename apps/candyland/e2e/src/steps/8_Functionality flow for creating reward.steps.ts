...
import {CreateRewardAppPage, ElementApp} from '../pages/candylandApp.po';
...
Then(/^6_The relevant text input fields are present.$/, async () => {
  ...
  await browser.wait(ec.presenceOf(CreateRewardAppPage.voucherCodeField()), 5000);
  ...
  expect(await CreateRewardAppPage.voucherCodeField().isPresent()).to.equal(true);
