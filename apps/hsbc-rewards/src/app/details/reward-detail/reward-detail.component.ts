  ...
  public ngOnInit(): void {
    ...
    this.displayPriceFn = (rewardPrice: IPrice) => {
      ...
      if (rewardPrice.points && rewardPrice.points > 0 && rewardPrice.price && rewardPrice.price > 0) {
        return of(`Fast Track: ${rewardPrice.points} points + ${rewardPrice.currencyCode} ${Math.floor(rewardPrice.price)}`);
      }

      if (rewardPrice.price && rewardPrice.price > 0) {
        return of(`${rewardPrice.currencyCode} ${Math.floor(rewardPrice.price)}`);
      }
